...
jsPsych.plugins["evan-run-trial"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    //      console.log('end of stage 2')
          if (recognition_trial){
            wait_for_time(0,function(){trial_master(next_stage_number)});
          }else{
          ...
          rafID2 = window.requestAnimationFrame(function(timestamp) {
            d3.selectAll('.choice_stim')
              ...
              if (!recognition_trial){
                ...
                frame_count_stage = Math.round(recognition_choice_stim_time / estimated_frame_duration); // max choice time
                frame_count = 0;
                next_stage_fun = function(){remove_choice()}; // change this to the recognition screen function
                //if (count_time == true){
                window.requestAnimationFrame(check_timeout);
              }
  ...
  //      console.log('response heard')
    ...
    var remove_choice = function(){
      ...
      if (recognition_trial){
        d3.selectAll('.choice_stim').call(setupMT).transition()
          .style("opacity",0).duration(0).on('end', this_MT)
      }else{
        d3.selectAll('.choice_stim').call(setupMT).transition()
    ...
    var display_outcome_recognition = function(){
      // this will display the outcome based on the response, which is a global variable
      //response.choice = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(response.key);
      var handle_response_recognition = function(info){

        console.log('handle response recognition')
        // kill the wait...
  //      console.log('response heard')
        count_time = false
        window.cancelAnimationFrame(rafID1);
        ...
        // want to add something showing that their choice was registered? - maybe change the background color?
        this_next_fun = function(){stage_3_master(3);}
        wait_for_time(500, this_next_fun);
        //stage_2_master(2);

      } // end handle_response_recognition
  ...
  var end_trial = function(){
    ...
    var trial_data = { // need to add timing to this.
      ...
      "correct_recognition": response.correct,
      "recognition_choice": response.recognition_choice,
      "recognition_number": trial.recognition_number
  ...
  var response = {
      ...
      choice:null,
      accept:null,
      correct_recognition: null,
      recognition_choice: null
    };
