...
for (var i = 0; i < n_rounds; i++){
  ...
  var schematic = {
	...
	var quiz_trials = make_struc_quiz_block(i + 1, 6, false); // start to limit time after 5 blocks... // want this to be adaptive though...
...
var build_play_machine_round = function(){
...
// goes through 6 quizzes, 3 experience rounds...
  for (var i = 0; i < 6; i++){
    var schematic1 = {
      type: 'evan-display-map',
      choice_images: choice_images,
    ...
    intro_w_trials.push(schematic1);

    if (i%2 == 0){ // if it's an even round...

      intro_w_trials.push(build_text_trial("You'll now have a chance to play each slot machine.","","", false))
      ...
      intro_w_trials.push(schematic2);
    }

    if (i < 3){ // the first 3 take about 12 minutes... so these should be 2 rounds
      intro_w_trials.push(build_text_trial("You'll now be quizzed on the chances of each slot machine producing either banknote.","Please try your best.","", false))

      var quiz_trials = make_struc_quiz_block(i + 1, 6, false);
  ...
  // goes through 8 quizzes, 4 map rounds
    for (var i = 0; i < 12; i++){
      ...
      if (i%2 == 0){ // if it's an even round...
        ...
        intro_w_trials.push(build_text_trial("You'll now be quizzed on the chances of each slot machine producing either banknote.","You now must respond quickly (within 3 seconds).","", false))
        var quiz_trials = make_struc_quiz_block(i + 1, 6, true);
        intro_w_trials = intro_w_trials.concat(quiz_trials);
    }
