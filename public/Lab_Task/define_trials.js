  ...
  for (var w_idx = 0; w_idx < all_win_amounts.length; w_idx++){
    for (var p_idx = 0; p_idx < all_prob_trig.length; p_idx++){
      ...
      /// gen test trial... function(o1_trig, prob_trig_idx, trig_val, matched_safe, safe_val_base, trigger_noise, other_noise, safe_noise){
          var const_add = round2(Math.ceil(20*Math.random()));
          var tn = const_add + round2(Math.ceil(8*Math.random() - 4));
          var on = const_add + round2(Math.ceil(8*Math.random() - 4));
...
//  }
  ...
  var splash_screen2 = {
  	...
  	var finish_instruc2_screen = {
  	    ...
  	    is_html: true,
  	    stimulus: 'You passed the quiz! Great work. The rest of the task will take about 30 minutes. Press the button to begin.'
  	}
...
timeline.push(end_screen);
 ...
 //timeline = [];
  //timeline.push(full_screen);
  //timeline = timeline.concat(task2_timeline.slice(60,70));
//timeline.push(end_screen);
